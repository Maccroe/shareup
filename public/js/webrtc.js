...
class WebRTCManager {
  constructor() {
    ...
    console.log(`[WebRTC] Device: iOS=${this.isIOS}, Safari=${this.isSafari}, iOS Safari=${this.isIOSSafari}`);
  }
  ...
  handleFileInfo(message) {
    // iOS Safari: stream chunks directly to IndexedDB to prevent RAM overflow
    // Android/Others: can buffer in memory but still save to IndexedDB
    ...
    console.log(`[WebRTC] Receiving: ${message.file.name} (${(message.file.size / 1024 / 1024).toFixed(2)}MB) - Streaming mode: ${useIndexedDBStreaming ? 'IndexedDB' : 'Memory'}`);
  ...
  async sendFile(file, transferId) {
    ...
    console.log(`[WebRTC] Sending file (chunk: ${chunkSize}B) as: ${isPremium ? 'PREMIUM' : isLoggedIn ? 'LOGGED-IN' : 'ANONYMOUS'} [Device: ${this.isIOSSafari ? 'iOS Safari' : 'Other'}]`);
    ...
    console.log(`[WebRTC] Backpressure: HIGH=${HIGH_WATER_MARK}B, LOW=${LOW_WATER_MARK}B`);
